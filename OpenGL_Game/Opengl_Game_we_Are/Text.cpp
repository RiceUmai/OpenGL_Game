...
void Text::Draw(Shader& shader, glm::vec3 Position, float x, float y, float scale, glm::vec3 color)
{
}
...
void Text::Draw(Shader& shader, float textF_x, float textF_y, float textF_z, float x, float y, float scale, glm::vec3 color)
{
	shader.use();
	shader.setVec3("textColor", color.x, color.y, color.z);
	glActiveTexture(GL_TEXTURE0);
	...
	std::string::const_iterator c;
	//for (c = text.begin(); c != text.end(); c++)
	std::string text = std::to_string(textF_x) + std::to_string(textF_y) + std::to_string(textF_z);
	for (c = text.begin(); c != text.end() - 5; c++)
	{
		Character ch = Characters[*c];
